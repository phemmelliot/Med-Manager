...
import static com.example.android.phemmelliot.phemmelliot.med_manager.TestUtils.getCurrentActivity;
...
@LargeTest
public class MedicationsScreenTest {
     ...
     */
    @Rule
    public ActivityTestRule<MedicationsActivity> mTasksActivityTestRule =
            new ActivityTestRule<MedicationsActivity>(MedicationsActivity.class) {

                /**
                 * To avoid a long list of medications and the need to scroll through the list to find a
                 * task, we call {@link MedicationsDataSource#deleteAllMessages()} before each test.
                 */
                @Override
    ...
    public void clickAddTaskButton_opensAddTaskUi() {
        ...
        onView(withId(R.id.fab_add_medication)).perform(click());

        // Check if the add task screen is displayed
        onView(withId(R.id.add_medication_name)).check(matches(isDisplayed()));
    }
    ...
    public void editTask() throws Exception {
        ...
        onView(withId(R.id.add_medication_name))
                .perform(replaceText(editTaskTitle), closeSoftKeyboard()); // Type new task title
        onView(withId(R.id.add_medication_description)).perform(replaceText(editTaskDescription),
    ...
    public void showAllTasks() {
        // Add 2 active medications
        createTask(TITLE1, DESCRIPTION);
        createTask(TITLE2, DESCRIPTION);

        //Verify that all our medications are shown
        viewAllTasks();
        onView(withItemText(TITLE1)).check(matches(isDisplayed()));
    ...
    public void clearCompletedTasks() {
        ...
        //Verify that completed medications are not shown
        onView(withItemText(TITLE1)).check(matches(not(isDisplayed())));
    ...
    public void orientationChange_FilterActivePersists() {
        ...
        // when switching to active medications
        viewActiveTasks();

        // then no medications should appear
        onView(withText(TITLE1)).check(matches(not(isDisplayed())));
    ...
    public void orientationChange_FilterCompletedPersists() {
        ...
        // when switching to completed medications
        viewCompletedTasks();
    ...
    public void orientationChange_DuringEdit_ChangePersists() throws Throwable {
        ...
        onView(withId(R.id.add_medication_name)).check(matches(withText(TITLE2)));
    }
    ...
    public void orientationChange_DuringEdit_NoDuplicate() throws IllegalStateException {
        ...
        onView(withId(R.id.add_medication_name))
                .perform(replaceText(TITLE2), closeSoftKeyboard()); // Type new task title
        onView(withId(R.id.add_medication_description)).perform(replaceText(DESCRIPTION),
                closeSoftKeyboard()); // Type new task description and close the keyboard
    ...
    private void createTask(String title, String description) {
        ...
        onView(withId(R.id.add_medication_name)).perform(typeText(title),
                closeSoftKeyboard()); // Type new task title
        onView(withId(R.id.add_medication_description)).perform(typeText(description),
                closeSoftKeyboard()); // Type new task description and close the keyboard
